...
function formatProduct(product, quantity = undefined) {
 if (quantity != undefined) {
  return product.title + " " + quantity + "Stk." + "- " + formatPrice(quantity*product.price);
 } 
 else{
...
function cartInfo(cart) {
  ...
  for (let i = 0; i < cart.lines.length; i++) {
    ...
    for (const line of cart.lines) {
      const product = products.find(e => e.id === line.product);
      returnString += `${formatProduct(product)}\n`;
      total += line.quantity * product.price;
    }
  
    return `${returnString}Samtals: ${formatPrice(total)}`;
...
function showProducts() {
 for (let i = 0; i < products.length; i++) {
  ...
  console.log(i+1 + " - " + product.title + " - " + product.description + " - " + formatPrice(product.price));
...
function checkout() {
  if(cart.lines.length < 1){
    return console.error("Karfan er tóm")

  }
  var nafn = prompt("Nafn")
  if(!nafn){
    return console.error("Það má ekki skilja nafn eftir tómt, vinsamlegast reyndu aftur")

  }
  var heimilisfang = promt("Heimilistfang:")
  if(!heimilisfang){
    return console.error("Það má ekki skilja heimilisfang eftir tómt, vinsamlegast reyndu aftur")
  }

  console.log("Pöntun mótekkin " + nafn + " vörur verða sendar á " + heimilisfang)
  cartInfo(cart)
